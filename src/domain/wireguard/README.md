...
| `data.network.post_up` | PostUp 命令（字符串或字符串数组），接口启动后执行 |
| `data.network.post_down` | PostDown 命令（字符串或字符串数组），接口关闭前执行 |
| `data.network.peers[]` | Peer 列表 |
| `data.status[]` | Agent 上报的状态数组，每轮 poll 更新 |
...
Agent 每轮 poll 通过 `PATCH /api/v1/vms/{name}/inventory/wireguard_network.json` 上报 `data.status`：

```json
// wg0 down
{"data": {"status": [{"key": "state", "value": "down"}]}}
