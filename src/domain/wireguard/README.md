...
| 字段 | 类型 | 必填 | 说明 |
|------|------|------|------|
| `publicKey` | string | 是 | Peer 的 WireGuard 公钥，44 字符 base64 |
| `endpoint` | string | 是 | Peer 的可达地址 `host:port` |
| `ip` | string | 否 | 快捷方式：单个 VPN IP（CIDR），等价于 `allowed_ips: [ip]` |
| `allowed_ips` | list[string] | 否 | 通过该 Peer 可达的 IP 范围列表（CIDR），如 `["10.200.0.2/32", "10.200.0.0/24"]`。不设时默认使用 `ip` |
